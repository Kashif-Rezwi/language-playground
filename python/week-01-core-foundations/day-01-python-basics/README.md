...
[X] I completed current day's exercises and practice scripts.
```
...
Move to Day 02: Strings & Manipulation. Focus on cleaning text with `.strip()`, standardizing casing with `.lower()` and `.title()`, slicing strings, and using f-strings for readable output.
